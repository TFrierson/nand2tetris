...
void Parser::advance()
{
	...
	{		
		...
		while (currentCommand.substr(0, 2) == "//" || currentCommand.length() == 0 && hasMoreCommands())
		{
			std::getline(inputFile, currentCommand);
...
std::string Parser::commandType()
{
	...
	//if there is no space in the line, then it is an arithmetic or a return command
	if (spacePos == std::string::npos && currentCommand != "return")
	{
		return "C_ARITHMETIC";
	}
